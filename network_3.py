...
class MPLS:
    ...
    def __init__(self, packet_in, label):
        new_string = packet_in.to_byte_S()
        self.label = label
...
#        print("new_packet = %s\n\n" % self.new_packet)
    ...
    def from_byte_S(self, byte_S):
        networkInnerPacket = byte_S[2:]
        networkPacket = NetworkPacket.from_byte_S(networkInnerPacket)
        label = int(byte_S[0:2])
        return self(networkPacket, label)
...
#                print 'MPLS QUEUE ' + pkt_S
    ...
    def process_network_packet(self, pkt, i, isNotUsingPriorities):
...
#        print("pkt.to_byte_S = %s\ni = %s\npkt.dst = %s\n" % (pkt.to_byte_S(), str(i), str(pkt.dst)))
        if isNotUsingPriorities:
            out_label = self.encap_tbl_D[i][pkt.dst]
            print('%s: NetworkPacket %s was not encapsulated with a priority label' % (str(self), str(pkt)))
        else:
            out_label = pkt.priority
            print('%s: NetworkPacket %s was encapsulated with a priority label' % (str(self), str(pkt)))
            ...
            for element in self.intf_L[i].in_queue.queue:
                if element[0] == 'M':
                    ...
                    if int(element[1]) == 0:
                        ...
                        priority1 = priority1 + 1
                elements += elements + 1
        if priority0 != 0 or priority1 != 0:
            print('%s: Priority 0: %s Priority 1: %s' % (str(self), str(priority0), str(priority1)))
            return priority0 + priority1 == elements
